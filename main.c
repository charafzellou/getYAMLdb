...
** Made by : Urrego.j
**
** Description : this file is used to test functions in yamlParser.c
*/
...
void mainMenu(char * dbName){
    ...
    do {
        ...
        if ( strcmp(sqlQuery, "quit") ){
           // sqlQuery(dbPath, sqlQuery, strlen(sqlQuery));
        }
    } while ( strcmp(sqlQuery, "quit") );

}

/*void sqlQuery(char * dbPath, char * sqlQuery, int querySize){
    char * pointerSave;
    char * table;
    for ( counter = 0; counter < querySize; counter++){
        switch (sqlQuery[counter]){
            case 'I':
            case 'i':
